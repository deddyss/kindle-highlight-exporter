...
export const showPreferences = (preferences: Preferences | null): void => {
	if (preferences) {
		const { lastSelectedBooks, ...restPreferences } = preferences;
		console.log("  " +
			chalk.cyan(
...
export const savePreferences = (answer: Answer): void => {
	const { useChrome, amazonRegion, selectedBooks } = answer;
	const preferences: Preferences = { useChrome, amazonRegion, lastSelectedBooks: selectedBooks };
